# The Inflated Geometric Distribution ------------------------------------------

#' @name rige
#'
#' @title The Reparameterized Inflated Geometric Distribution
#'
#' @description Probability mass function, distribution function, quantile function, and random generation
#' for the reparameterized inflated geometric (RIGE) distribution with mean \code{mu} and
...
#' @param sigma2 numeric; variance (greather than \code{mu * (mu + 1)}),
...
#'    added to the rigpsreg package. Not used in RIGE distribution.
#'
#' @return \code{drige} returns the probability mass function, \code{prige} gives the distribution
#'     function, \code{qrige} gives the quantile function, and \code{rrige} generates random
#'     observations.
#'
#'     \code{rige} returns a \code{"count"} object mainly used to fit the RIGE distribution
...
#'    distribution function, quantile function, and a random number generator for the RIGE
...
#' points(xcoords, drige(yvals, mu, sigma2), pch = 16, col = 2, type = "b")
#'
#' # distribution function theoretical vs empirical comparison
#' plot(ecdf(y), xlab = "y", ylab = "Cdf", main = " ")
#' curve(prige(x, mu, sigma2), add = TRUE, col = 2, lwd = 2)
#'
...
#' curve(qrige(x, mu, sigma2), add = TRUE, col = 2, lwd = 2)


## Probability mass function
#' @rdname rige
#' @export
drige <- function(x, mu, sigma2, log.p = FALSE, ...){
  ...
  # NaN index
  pmf[which(mu <= 0 | sigma^2 <= mu * (mu + 1), arr.ind = TRUE)] <- NaN
  ...
  pmf[id2] <- log(4) + 3 * log(mu[id2]) - 2 * log(sigma[id2]^2 + mu[id2]^2 + mu[id2]) +
    (x[id2] - 1) * log( (4 * mu[id2]^3) / ((sigma[id2]^2 + mu[id2])^2 - mu[id2]^4) +
                          (mu[id2] * (mu[id2] + 1) - sigma[id2]^2) /
                          (mu[id2] * (mu[id2] - 1) - sigma[id2]^2) )
...
#' @rdname rige
#' @export
prige <- function(q, mu, sigma2, lower.tail = TRUE, log.p = FALSE, ...){
  ...
  # NaN index
  cdf[which(mu <= 0 | sigma^2 <= mu * (mu + 1), arr.ind = TRUE)] <- NaN

  # Positive pmf index
  id <- which(q >= 0 & !is.nan(cdf), arr.ind = TRUE)

  cdf[id] <- 1 - ((2 * mu[id]^2) / (sigma[id]^2 + mu[id] + mu[id]^2)) *
    (4 * mu[id]^3 / ((sigma[id]^2 + mu[id])^2 - mu[id]^4) +
       (mu[id] * (mu[id] + 1) - sigma[id]^2) /
       (mu[id] * (mu[id] - 1) - sigma[id]^2))^floor(q[id])
...
#' @rdname rige
#' @export
qrige <- function(p, mu, sigma2, lower.tail = TRUE, ...){
  ...
  # NaN index
  qtl[which(mu <= 0 | sigma^2 <= mu * (mu + 1), arr.ind = TRUE)] <- NaN
  ...
  qtl[id] <- ceiling(
    (log(1 - p[id]) - log(2 * mu[id]^2) + log(sigma[id]^2 + mu[id] * (mu[id] + 1))) /
      log(4 * mu[id]^3 / ((sigma[id]^2 + mu[id])^2 - mu[id]^4) +
            (mu[id] * (mu[id] + 1) - sigma[id]^2) /
            (mu[id] * (mu[id] - 1) - sigma[id]^2))
  )
...
#' @rdname rige
#' @export
rige <- function(mu.link = "log", sigma2.link = "log"){
  ...
  # Name
  out$name <- "Reparameterized inflated geometric"
  ...
  # Constraint -----------------------------------------------------------------
  out$constraint <- function(mu, sigma2, ...) (mu > 0) & (sigma2 >= mu * (mu + 1))
  ...
  out$start <- function(x, X, Z){
    ...
    sigma0 <- stats::var(x) + max(mu0) * (max(mu0) + 1)
  ...
  out$dl.dsigma <- function(x, mu, sigma2, ...){
    ...
    # First
    dcMS.dsigma <- 4 * mu / ((mu^2 + mu + sigma^2)^2)
...
# print(paste("sigma2:", round(sigma2, 3), "sample var:", round(var(y), 3)))
#
# par(mfrow = c(1, 3))
# yvals <- sort(unique(y))
...
# n <- 100
#
# set.seed(12); X <- cbind(1, runif(n), rbinom(1, 1, 0.6))
# set.seed(12); Z <- cbind(1, runif(n))
#
# beta <- c(1.5, -0.6, -0.7)
# gamma <- c(2, 0.5)
#
...
#
# all(rige()$constraint(mu, sigma2))
#
# y <- matrix(NA, 1000, n)
# for(i in 1:1000){
#   y[i, ] <- rrige(n, mu, sigma2)
#   if(i %% 100 == 0) print(i)
# }
#
# i <- sample(1:n, 1)
...
# points(xcoords, drige(yvals, mu[i], sigma2[i]), pch = 16, col = 2, type = "b")
#
# plot(ecdf(y[, i]), xlab = "y", ylab = "Cdf", main = paste0("Replicate: ", i))
# curve(prige(x, mu[i], sigma2[i]), add = TRUE, col = 2, lwd = 2)
#
# plot(seq(0.01, 0.99, 0.001), quantile(y[, i], seq(0.01, 0.99, 0.001)),
#      pch = 16, xlab = "p", ylab = "Quantile")
# curve(qrige(x, mu[i], sigma2[i]), add = TRUE, col = 2, lwd = 2)
# par(mfrow = c(1, 1))
#
#
...
# n <- 500
# mu <- runif(1, 1, 6)
# sigma2 <- runif(1, mu * (mu + 1), mu * (mu + 1) + 1)
#

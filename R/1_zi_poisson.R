...
#' @name zip
#'
#' @title The Reparameterized Zero-Inflated Poisson Distribution
#'
#' @description Probability mass function, distribution function, quantile function, and random generation
#' for the reparameterized zero-inflated Poisson (ZIP) distribution with mean \code{mu} and
...
#'    added to the \code{rigpsreg} package. Not used in ZIP distribution.
#'
#' @return \code{dzip} returns the probability mass function, \code{pzip} gives the distribution
#'     function, \code{qzip} gives the quantile function, and \code{rzip} generates random
#'     observations.
#'
#'     \code{zip} returns a \code{"count"} object mainly used to fit the ZIP distribution
...
#'    distribution function, quantile function, and a random number generator for the ZIP
#'    distribution parameterized in terms of the mean (\code{mu}) and the variance (\code{sigma2}).
#'    This parameterization was introduced by Rodríguez-Avi and Olmo-Jiménez (2017) and
#'    also applied by Kokonendji, Medeiros, and Bourguignon (2024).
#'
#' @references Rodríguez-Avi, J., and Olmo-Jiménez, M. J. (2017).
...
#' points(xcoords, dzip(yvals, mu, sigma2), pch = 16, col = 2, type = "b")
#'
#' # distribution function theoretical vs empirical comparison
#' plot(ecdf(y), xlab = "y", ylab = "Cdf", main = " ")
#' curve(pzip(x, mu, sigma2), add = TRUE, col = 2, lwd = 2)
#'
...
#' curve(qzip(x, mu, sigma2), add = TRUE, col = 2, lwd = 2)


## Probability mass function
#' @rdname zip
#' @export
dzip <- function(x, mu, sigma2, log.p = FALSE, ...){
  ...
  lambda <- exp(log(sigma^2 + mu^2 - mu) - log(mu))
  phi <- exp(log(sigma^2 - mu) - log(sigma^2 + mu^2 - mu))

  pmf[id] <- log(1 - phi[id]) + x[id] * log(lambda[id]) - lambda[id] - lfactorial(x[id])

  id0 <- which(!is.nan(pmf) & x == 0, arr.ind = TRUE)
  pmf[id0] <- log(phi[id0] + (1 - phi[id0]) * exp(-lambda[id0]))
...
#' @rdname zip
#' @export
pzip <- function(q, mu, sigma2, lower.tail = TRUE, log.p = FALSE, ...)
{
  sigma <- sqrt(sigma2)
  ...
  # NaN index
  cdf[which(mu <= 0 | sigma^2 <= mu, arr.ind = TRUE)] <- NaN

  # Positive pmf index
  id <- which(q >= 0 & !is.nan(cdf), arr.ind = TRUE)

  lambda <- (sigma^2 + mu^2 - mu) / mu
  phi <- (sigma^2 - mu) / (sigma^2 + mu^2 - mu)
  cdf[id] <- gamlss.dist::pZIP(q = q[id], mu = lambda[id], sigma = phi[id])
...
#' @rdname zip
#' @export
qzip <- function(p, mu, sigma2, lower.tail = TRUE, ...)
{
  sigma <- sqrt(sigma2)
  ...
  # NaN index
  qtl[which(mu <= 0 | sigma^2 <= mu, arr.ind = TRUE)] <- NaN

  lambda <- (sigma^2 + mu^2 - mu) / mu
  phi <- (sigma^2 - mu) / (sigma^2 + mu^2 - mu)
  id <- which(!is.nan(qtl), arr.ind = TRUE)
  qtl[id] <- gamlss.dist::qZIP(p[id], mu = lambda[id], sigma = phi[id])
...
#' @rdname zip
#' @export
rzip <- function(n, mu, sigma2, ...){
  sigma <- sqrt(sigma2)
  lambda <- (sigma^2 + mu^2 - mu) / mu
  phi <- (sigma^2 - mu) / (sigma^2 + mu^2 - mu)
  gamlss.dist::rZIP(n, mu = lambda, sigma = phi)
}

#' @rdname zip
#' @export
zip <- function(mu.link = "log", sigma2.link = "log"){
  ...
  # Name
  out$name <- "Reparameterized zero-inflated Poisson"
...
# n <- 10000
# mu <- runif(1, 1, 6)
# sigma2 <- round(runif(1, sqrt(mu), 8), 3)
# y <- rzip(n, mu, sigma2)

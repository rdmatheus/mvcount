# The Reparameterized Double Poisson Distribution -------------------------------------

#' @name dbpo
#'
#' @title The Reparameterized Double Poisson Distribution
#'
#' @description Probability mass function, distribution function, quantile function, and random generation
#' for the reparameterized double Poisson distribution with mean \code{mu} and
...
#'    added to the \code{rigpsreg} package. Not used in double Poisson distribution.
#'
#' @return \code{ddbpo} returns the probability mass function, \code{pdbpo} gives the distribution
#'     function, \code{qdbpo} gives the quantile function, and \code{rdbpo} generates random
#'     observations.
#'
#'     \code{dbpo} returns a \code{"count"} object mainly used to fit the double Poisson distribution
...
#'    distribution function, quantile function, and a random number generator for the double Poisson
...
#' points(xcoords, ddbpo(yvals, mu, sigma2), pch = 16, col = 2, type = "b")
#'
#' # distribution function theoretical vs empirical comparison
#' plot(ecdf(y), xlab = "y", ylab = "Cdf", main = " ")
#' curve(pdbpo(x, mu, sigma2), add = TRUE, col = 2, lwd = 2)
#'
...
#' curve(qdbpo(x, mu, sigma2), add = TRUE, col = 2, lwd = 2)

## Probability mass function
#' @rdname dbpo
#' @export
ddbpo <- function(x, mu, sigma2, log.p = FALSE, ...){
...
#' @rdname dbpo
#' @export
pdbpo <- function(q, mu, sigma2, lower.tail = TRUE, log.p = FALSE, ...)
{
...
#' @rdname dbpo
#' @export
qdbpo <- function(p, mu, sigma2, lower.tail = TRUE, ...)
{
...
#' @rdname dbpo
#' @export
rdbpo <- function(n, mu, sigma2, ...){

  phi <- sigma2 / mu

  gamlss.dist::rDPO(n, mu = mu, sigma = phi)

}

#' @rdname dbpo
#' @export
dbpo <- function(mu.link = "log", sigma2.link = "log"){
